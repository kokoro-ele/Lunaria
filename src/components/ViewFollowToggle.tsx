...
export default function ViewFollowToggle({ active, onToggle }: ViewFollowToggleProps) {
  ...
  return (
    <div className="group relative">
      <button
        onClick={onToggle}
        className="panel flex w-full min-w-0 items-center gap-2 px-3 py-2 text-[10px] font-mono uppercase tracking-widest2 text-white/60 transition-all hover:border-white/35 hover:text-white/90 md:w-auto md:gap-2.5 md:px-4 md:py-2.5 md:text-[11px]"
        aria-pressed={active}
        title={t('controls.tiltTooltip')}
      >
        <Dot on={active} />
        <span className="truncate">{t('controls.viewFollow')}</span>
